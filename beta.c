...
int menu(int lin1, int col1, int qtd, char lista[3][40]){
    ...
    setlocale(LC_ALL, "");
    //laÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¿Ãƒâ€šÃ‚Â½o das opcÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¿Ãƒâ€šÃ‚Â½es
    while(1){
        ...
        tecla = getch();
        linhaCol(22, 1);
        //OpÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¿Ãƒâ€šÃ‚Â½ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¿Ãƒâ€šÃ‚Â½o
        if (tecla == 27){               //ESC
            opc = 0;
            break;
            ...
            if(opc > 1){
                opc--;                  // se a opÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o for maior que 1, pode voltar 
            }
        } else if (tecla == 115 || tecla == 83){        //S
            if (opc < qtd){
                opc++;                  //Se a opÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o for menor que quantidade de itens, posso avanÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ar    
            }
        }
    }
    return opc;
...
int TpRandom(){
	srand(time(NULL));
	do {
		//fase3[random_i][random_j] = fase3[rand() % 59][rand() % 59];
		random_i = rand() % 59;
		random_j = rand() % 59;
...
void TerceiraFase(){
	...
	while(1){
		...
		switch (toupper(input)){
			...
			case 'D':
				if (fase3[player_i][player_j + 1] == '>' && player_j + 1 == 3){
					TpRandom();
